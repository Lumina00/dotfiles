...
sudo pacman -S --noconfirm --needed rustup cmake docker nodejs \
  zsh refind mpv gst-plugins-{base,good,ugly,bad} firefox-developer-{edition,edition-i18n-ja} qbittorrent neovim xclip wl-clipboard rsync eza bat\
...
if [ "$1" == "1"]; then 
	...
	pacman -S --noconfirm --need hyprland hypridle ruby jq bluetoothctl grim slurp mpd brightlessctl dunst swww
fi 
exit
